...
func FilterWildCard(results []result.Result) []result.Result {
	...
	for _, res := range results {
		for _, answer := range res.Answers {
			// 跳过非IP的记录(CNAME等)
			if !strings.HasPrefix(answer.Type, "CNAME ") && !strings.HasPrefix(answer.Type, "NS ") &&
				!strings.HasPrefix(answer.Type, "TXT ") && !strings.HasPrefix(answer.Type, "PTR ") {
				ipFrequency[answer.Value[0]]++
				ipToDomains[answer.Value[0]] = append(ipToDomains[answer.Value[0]], res.Subdomain)
			}
		}
	}
	...
	for _, res := range results {
		...
		for _, answer := range res.Answers {
			...
			} else if !suspiciousIPs[answer.Value[0]] {
				// 保留不在可疑IP列表中的IP
				validRecord = true
				filteredAnswers = append(filteredAnswers, answer)
			}
...
func FilterWildCardAdvanced(results []result.Result) []result.Result {
	...
	for _, res := range results {
		...
		for _, answer := range res.Answers {
			if strings.HasPrefix(answer.Type, "CNAME ") {
				// 提取CNAME目标
				cnameParts := strings.SplitN(answer.Value[0], " ", 2)
				if len(cnameParts) == 2 {
			...
			// 只处理IP记录
			if !strings.HasPrefix(answer.Type, "NS ") &&
				!strings.HasPrefix(answer.Type, "TXT ") &&
				!strings.HasPrefix(answer.Type, "PTR ") {
				// 计数IP频率
				ipFrequency[answer.Value[0]]++

				// 初始化IP的前缀集合和TLD集合
				if ipPrefixVariety[answer.Value[0]] == nil {
					ipPrefixVariety[answer.Value[0]] = make(map[string]bool)
				}
				if ipTLDVariety[answer.Value[0]] == nil {
					ipTLDVariety[answer.Value[0]] = make(map[string]bool)
				}

				// 记录这个IP解析了哪些不同的前缀和TLD
				ipPrefixVariety[answer.Value[0]][prefix] = true
				ipTLDVariety[answer.Value[0]][tld] = true

				// 记录IP到域名的映射
				ipToDomains[answer.Value[0]] = append(ipToDomains[answer.Value[0]], subdomain)
			}
		}
	}
	...
	for _, res := range results {
		...
		// 处理所有回答
		for _, answer := range res.Answers {
			isIP := !strings.HasPrefix(answer.Type, "CNAME ") &&
				!strings.HasPrefix(answer.Type, "NS ") &&
				!strings.HasPrefix(answer.Type, "TXT ") &&
				!strings.HasPrefix(answer.Type, "PTR ")
			...
			if !isIP {
				if strings.HasPrefix(answer.Type, "CNAME ") {
					cnameParts := strings.SplitN(answer.Type, " ", 2)
				...
				// 针对IP记录，根据可疑度评分过滤
				suspiciousScore, isSuspicious := suspiciousIPs[answer.Value[0]]
